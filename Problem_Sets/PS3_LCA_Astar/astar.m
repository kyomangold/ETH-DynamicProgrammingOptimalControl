...
function [optimal_cost, optimal_path] = astar(A,start_node,terminal_node)
...
while 1
    ...
    for j = children
        % STEP 2: If d_i + a_ij < and  d_i + a_ij + h_j < shortest_path, set d_j = d_i + a_ij and set i to be the parent of j.
        if (d(i) + A(i,j) < d(j) && d(i) + A(i,j) + abs(j-terminal_node) < shortest_path)
            d(j) = d(i) + A(i,j);

            parent(j) = i;
            
            % In addition, if j ~= t, place j in candidates if it is not already in candidates, while if j == t, set shortest_path to the new value d_i + a_it of d_t
            if (j ~= terminal_node)
                if ~(candidates == j)
                    last_candidate = last_candidate + 1;
